	...
	<view class="page-container">
		...
		<view class="bottom-bar">
			<view class="bottom-left">
				...
				<view class="bottom-icon-btn wish-btn" @click="toggleWishlist">
					<uni-icons :type="isInWishlist ? 'heart-filled' : 'heart'" size="24" :color="isInWishlist ? '#E74C3C' : '#6B5B4F'"></uni-icons>
				</view>
			</view>
				...
				<view class="cart-header" @click="toggleCartList">
					<view class="cart-info">
						...
						<view class="cart-detail">
							...
							<text class="cart-count">共 {{ cartTotal.totalCount }} 本</text>
						</view>
					</view>
...
<script>
	import { getCart, getCartTotal, addToCart as cartAdd, updateCartItemQuantity, clearCart, removeFromCart } from '@/utils/cart.js';
	...
	export default {
		...
		onLoad(options) {
			...
			if (coffeeData) {
				this.product = {
					...
					stock: coffeeData.stock !== undefined ? coffeeData.stock : 0
				};
			}
			this.updateCartTotal();
		},
		methods: {
			...
			loadWishlist() {
				try {
					this.wishlist = uni.getStorageSync('wishlist') || [];
				} catch (e) {
					this.wishlist = [];
				}
			},
			toggleWishlist() {
				if (!this.product._id) {
					uni.showToast({ title: '书籍信息加载中', icon: 'none' });
					return;
				}
				const idx = this.wishlist.findIndex(item => item._id === this.product._id);
				if (idx > -1) {
					this.wishlist.splice(idx, 1);
					uni.showToast({ title: '已移出心愿书单', icon: 'none' });
				} else {
					this.wishlist.push({
					...
					uni.showToast({ title: '已加入心愿书单', icon: 'success' });
				}
				uni.setStorageSync('wishlist', this.wishlist);
			},
			loadCartItems() {
				const cart = getCart();
			...
			addToBorrowList() {
				...
				if (existingItem) {
					if (existingItem.quantity >= Number(this.product.stock)) {
				...
				for (const item of cart) {
					for (let index = 0; index < item.quantity; index++) {
						try {
							...
							const remaining = item.quantity - index - 1;
							if (remaining > 0) {
